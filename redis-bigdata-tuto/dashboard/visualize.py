...
def collect_metrics():
    ...
    for i in range(60):
        ...
        if i % 10 == 0:
            print(f"  {i+1}/60 requêtes — hit ratio : {stats['hit_ratio_pct']}%")

    return cold_latencies, hot_latencies, hit_ratios


def build_dashboard(cold, hot, ratios):
    stats    = httpx.get(f"{BASE_URL}/stats").json()
    ...
    fig.suptitle("Redis Cache Dashboard — BigData MovieLens", fontsize=16, fontweight="bold", color="#e94560")
    ...
    ax.axhline(y=80, color=AMBER, linewidth=1, linestyle="--", alpha=0.6, label="Cible 80%")
    ...
    x = range(len(labels))
    w = 0.35
    bars1 = ax.bar([i - w/2 for i in x], cold_vals, w, label="MongoDB (miss)", color=CORAL, alpha=0.85)
    bars2 = ax.bar([i + w/2 for i in x], hot_vals,  w, label="Redis (hit)",    color=TEAL,  alpha=0.85)
    ...
    ax.set_title("Top 10 films consultés (Sorted Set)", color=PURPLE, fontsize=11)
    ...
    bars = ax.bar(mem_labels, mem_values, color=bar_colors, alpha=0.85, width=0.5)
    ...
    plt.tight_layout()
    output_path = "dashboard/redis_dashboard.png"
...
if __name__ == "__main__":
    ...
    build_dashboard(cold, hot, ratios)
